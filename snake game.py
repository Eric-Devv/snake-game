...
# DEVELOPED BY Eric.Dev IN COOPERATION WITH BRIGHTECH.SOLUTIONS
...
def welcome():
    ...
    while not game_exit:
        ...
        score_on_screen("The snake Game",white,272,250)
        score_on_screen("Press spacebar to play",white,232,290)
...
def game():
    ...
    while not game_exit:
        if game_over:
            ...
            score_on_screen("Game Over! Press Enter to continue",black,100,250)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    game_exit = True
                if event.type== pygame.KEYDOWN:
            ...
            if abs(snake_x - apple_x)<20 and abs(snake_y - apple_y)<20:
                ...
                if score>int(highscore):
                    highscore = score
            gameWindow.fill(white)
            score_on_screen("Score: "+str(score) + " highscore: "+str(highscore),green,5,5)
